...
- **The drill-down goes back** (added later) — `ScopeBannerView.onBack` draws a chevron in the header's leading slot, beside the avatar rather than replacing it, and Transactions supplies it while the ledger is still the slice it was handed (`isShowingHandedOverSlice` compares the live filter/period to the request rather than tracking a flag, so any control that undoes the hand-over clears the chevron for free). Going back is **only** `tab = .home`: all four tabs stay mounted, so `expandedId`, the expansion step and the widget's own direction and highlighted bucket were never torn down. There is nothing to restore, and anything that restored it would be a second source of truth.
