...
values ('e8000000-0000-0000-0000-00000000e001', auth.uid(), auth.uid(), 'ledger', 'checking', 'Ledger', 'EUR', 1000);
...
values (auth.uid(), auth.uid(), 'e8000000-0000-0000-0000-00000000e001', 'c8000000-0000-0000-0000-00000000c001', -100, 'EUR', now() - interval '5 days');
insert into transactions (owner_id, created_by, account_id, category_id, amount, currency, occurred_at)
values (auth.uid(), auth.uid(), 'e8000000-0000-0000-0000-00000000e001', 'c8000000-0000-0000-0000-00000000c001', -50, 'EUR', now());
...
select is(
  ...
  1000.0000,
  'account_balance_on before any transaction is exactly opening_balance'
);
...
select is(
  (select balance from account_balances where account_id = 'e8000000-0000-0000-0000-00000000e001'),
  account_balance_on('e8000000-0000-0000-0000-00000000e001', current_date),
...
insert into transactions (owner_id, created_by, account_id, category_id, amount, currency, occurred_at)
values (auth.uid(), auth.uid(), 'e8000000-0000-0000-0000-00000000e001', 'c8000000-0000-0000-0000-00000000c001', -9999, 'EUR', now() + interval '1 day');
...
select is(
  (select balance from net_worth_daily
...
select is(
  (select total from net_worth_series('household', current_date, current_date)),
  850.0000,
  'net_worth_series(''household'') totals the shared account correctly'
);

select is(
  (select total from net_worth_series('total', current_date, current_date)),
  850.0000,
...
select is(
  fx_convert(100, 'USD', 'USD', current_date),
  100.0000,
...
insert into accounts (id, owner_id, created_by, kind, subtype, name, currency, opening_balance)
values ('e8000000-0000-0000-0000-00000000e002', auth.uid(), auth.uid(), 'ledger', 'checking', 'USD Ledger', 'USD', 500);

select refresh_net_worth_daily(auth.uid(), current_date, current_date);

select is(
  (select total from net_worth_series('me', current_date, current_date)),
  null::numeric,
...
select is(
  (select count(*) from net_worth_series('me', (current_date - 8), (current_date - 1)) where total is null),
  0::bigint,
