...
values ('a3000000-0000-0000-0000-000000000001', auth.uid(), auth.uid(), 'ledger', 'checking', 'A Checking', 'EUR', 0);
...
values (
  ...
  'c3000000-0000-0000-0000-000000000001', -42.00, 'EUR', now() - interval '1 day'
);
...
select is(
  (
    select count(*) from import_csv_rows(
      ...
      jsonb_build_array(
        jsonb_build_object(
          'occurred_at', (now() - interval '1 day')::text, 'amount', '-42.00', 'currency', 'EUR',
          'merchant_raw', 'SUPERMARKET', 'merchant_normalized', 'SUPERMARKET'
        ),
        jsonb_build_object(
          'occurred_at', now()::text, 'amount', '-9.99', 'currency', 'EUR',
...
select is(
  (select matched_transaction_id from csv_import_candidates where amount = -42.00 and account_id = 'a3000000-0000-0000-0000-000000000001'),
  'd3000000-0000-0000-0000-000000000001'::uuid,
  'a row within 3 days at the exact amount matches the existing transaction'
);

select is(
  (select matched_transaction_id from csv_import_candidates where amount = -9.99),
  null::uuid,
...
select is(
  (select (accept_import_candidate(id)).conflict from csv_import_candidates where amount = -9.99),
  false,
  'accepting an unmatched candidate succeeds'
);

select is(
  (select count(*) from transactions where account_id = 'a3000000-0000-0000-0000-000000000001' and amount = -9.99 and source = 'csv_import'),
  1::bigint,
  'accepting a candidate inserts a confirmed transaction with source csv_import'
);

select is(
  (select status from csv_import_candidates where amount = -9.99),
...
select throws_ok(
  $$ select accept_import_candidate((select id from csv_import_candidates where amount = -9.99)) $$,
  null::char(5), null,
...
select reject_import_candidate((select id from csv_import_candidates where amount = -42.00));

select is(
  (select status from csv_import_candidates where amount = -42.00),
  'rejected'::import_candidate_status,
  'rejecting a matched (likely-duplicate) candidate flips it to rejected'
);

select is(
  (select count(*) from transactions where account_id = 'a3000000-0000-0000-0000-000000000001' and amount = -42.00 and source = 'csv_import'),
  0::bigint,
...
select import_csv_rows(
  ...
  jsonb_build_array(jsonb_build_object('occurred_at', now()::text, 'amount', '1000.00', 'currency', 'EUR'))
);
select is(
  (select count(*) from needs_review where kind = 'csv_import_candidate' and amount = 1000.00),
  1::bigint,
...
select is(
  (select diverges from check_transfer_rate_divergence('EUR', 'USD', 100, 90, now())),
  false,
...
select is(
  (select diverges from check_transfer_rate_divergence('EUR', 'USD', 100, 900, now())),
  true,
