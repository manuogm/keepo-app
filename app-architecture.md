...
- **Mango (`#FF9F1C`) is `BrandPrimary`, the app's only accent, and stays a UI accent** (the Needs Review inbox, the Pending badge, the offline bar) — it falls outside the lightness band needed for a data series on a white surface. There is no `BrandSecondary`; see `keepo-brand-identity.md`.
