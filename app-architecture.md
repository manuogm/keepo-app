...
| Home (Dashboard) | Net-worth card (title, balance, axis-free chart) pushing to a full-chart detail screen; Total / Me / Household picker; bell icon opens the Needs Review inbox as a floating notifications panel |
| Accounts | List, add/edit, icon + color picker, archive (with confirmation). "Everyday" (`ledger`) and "Investments" (`valuation`) sections; archived accounts collapse to an "Archived (N)" link into a separate screen (My Profile → Data & Privacy → Data) with unarchive and non-reversible delete |
...
| Transactions | Single day-grouped list with a day/week/month/year/custom period picker up top, plus an account/category/kind filter sheet |
...
- **`accounts`** — `id` (client-generated UUID), `owner_id`, `created_by`, `kind` (`account_kind`), `subtype` (`account_subtype`), `name`, `currency` (FK `currencies`), `opening_balance`, `opening_balance_at`, `include_in_total`, `icon`, `color` (presentational only, same pattern as `categories`), `archived_at`, `version`, `deleted_at`, timestamps. `UNIQUE (id, currency)` — the anchor for a composite FK below.
