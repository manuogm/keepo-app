...
| All Tags | A wrapping field of **pills** (the same `TagChip` the transaction form draws), reached from the bottom of the Categories tab. Everything is edited in place — tapping a pill puts the caret in it, because a tag has nothing else to edit and a form containing one text field would be a sheet over a screen already showing that field. Add is a dashed pill that becomes a field; delete is the shared `RemoveBadge` — the same red minus a dashboard tile wears in edit mode — revealed on the focused pill, since a flow of pills has no rows to swipe (the same reason the Categories grid moved delete into its edit sheet). A household member's shared tag draws as a plain chip: `tags_update` is owner-only, so it gets neither caret nor badge |
