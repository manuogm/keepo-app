...
1. `amount` is **signed** — negative outflow, positive inflow. Every account behaves identically: `regular` and `investment` are the only two kinds, both take income/expense/transfer, and balance is `opening_balance + SUM(amount)` for both — there is no second balance formula. `kind` never branches **balance computation** — no query, view, or RPC that computes money may test it. It is otherwise the user's own declaration of which accounts are investments: it drives the "Investment" badge, and display-only metrics that need to classify accounts (the Investing Ratio widget) read it rather than inventing a parallel flag. Never re-sign in application code.
