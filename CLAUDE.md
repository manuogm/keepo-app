...
1. `amount` is **signed** — negative outflow, positive inflow. Every account behaves identically: `regular` and `investment` are the only two kinds, both take income/expense/transfer, and balance is `opening_balance + SUM(amount)` for both — there is no second balance formula. A household partner on a share that starts on a date gets the same formula over an **adjusted opening**: `pull_changes` hands them an opening dated at the start day that holds the balance carried into it (`account_opening_as_seen`), computed in SQL on read and never stored. `kind` never branches **balance computation** — no query, view, or RPC that computes money may test it. It is otherwise the user's own declaration of which accounts are investments: it drives the "Investment" badge, and display-only metrics that need to classify accounts (the Investing Ratio widget) read it rather than inventing a parallel flag. Never re-sign in application code.
...
6. **Two different conversions exist; only one of them is ever stored.**
   - **Display conversion** — an account's currency → *the viewer's base currency*. **Never stored.** It derives from a value the user can change, so it is computed on read, every time, by `fx_convert`. `fx_rates` is append-only, so converting on read is stable *and* survives a base-currency change. This is the original rule, unchanged.
   - **Paid → account conversion** — the currency a purchase was *charged in* → the account's own currency. **Stored**, because it is a fact about what happened rather than a derivation: a €50 purchase on a USD account really did move some number of dollars. The account-currency figure is `amount_e4` (`balance = opening_balance + SUM(amount)` requires every row to be in the account's currency), and `original_amount_e4` + `original_currency` record what was actually paid. Those two are **provenance only: never summed, never re-converted, never a balance.** They are null whenever the purchase was already in the account's currency, so "is this foreign?" is a null check.
   - **The stored figure is prefilled and always editable, never a silent result.** Keepo's ECB reference rate is not the rate the bank used — Visa, Amex and Revolut each add a spread — so a silently stored reference conversion is an estimate wearing a fact's clothes, and because the balance is a running sum, every foreign purchase would drift the account away from reality permanently. The user overwrites the prefill with what their bank actually charged. **That property is the entire justification for storing this conversion; without it, this rule should read as it originally did.** No resolvable rate means rule 5 applies — the field renders `—` and the user types it. Never a guessed rate.
