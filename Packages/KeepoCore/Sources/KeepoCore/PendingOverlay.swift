...
public enum PendingOp: Sendable {
    ...
    case transactionDelta(accountId: UUID, amount: Int64)
    ...
    case transferDelta(fromAccountId: UUID, fromAmount: Int64, toAccountId: UUID, toAmount: Int64)
    ...
    case accountBalanceOverride(accountId: UUID, newBalance: Int64)
}
...
public enum PendingOverlay {
    ...
    public static func accountBalances(cached: [UUID: Int64], ops: [PendingOp]) -> [UUID: Int64] {
        var overrideBaseline: [UUID: Int64] = [:]
        var runningDelta: [UUID: Int64] = [:]

        func applyDelta(_ accountId: UUID, _ amount: Int64) {
